...
export default class AOEth extends AORouterInterface {
    ...
    _getHostContentEventForTransaction(request: IAORouterRequest) {
        ...
        this._listenForTransactionStatus(requestData.transactionHash).then(({status, receipt}) => {
            if ( status && receipt ) {
                // We can actually pull the event logs generated by this specific transaction
                const hostContentEvent: HostContentEvent = receipt.logs[0].args
                request.respond({
                    status, 
                    hostContentEvent,
                })
    ...
    _getStakeContentEventForTransaction(request: IAORouterRequest) {
        ...
        this._listenForTransactionStatus(requestData.transactionHash).then(({ status, receipt }) => {
            if (status && receipt) {
                ...
                const stakeContentEvent: StakeContentEvent = receipt.logs[1].args;
                const hostContentEvent: HostContentEvent = receipt.logs[2].args;
                request.respond({
                    status,
                    stakeContentEvent,
                    hostContentEvent,
