...
export default (
    ...
    return new Promise((resolve, reject) => {
        if (args.inputs.networkId != context.options.ethNetworkId) {
            ...
            context.router
                .send("/core/networkIdMismatch", networkIdMismatchData)
                .catch(debug);
        } else {
            resolveSetNetwork(obj, args, context, info)
                .then((ethNetworkConnected: boolean) => {
                    if (!ethNetworkConnected) {
                        reject(
                            new Error(`Unable to connect to ethereum network`)
                        );
                    } else {
                        context.userSession
