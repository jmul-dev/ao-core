...
export default (
    ...
    return new Promise((resolve, reject) => {
        ...
        const fileInputs = ["content", "videoTeaser", "featuredImage"];
        ...
        Promise.all(mkdirPromises)
            .then(() => {
                for (let i = 0; i < fileInputs.length; i++) {
                    ...
                    fileStorePromises.push(
                        new Promise((localResolve, localReject) => {
                            args.inputs[fileInputName]
                                .then(
                                    ({
                                        stream,
                                        filename,
                                        ...
                                        const writeStreamData: IAOFS_WriteStream_Data = {
                                            ...
                                            encrypt:
                                                fileInputName == "content"
                                                    ? true
                                                    : false,
                                            videoStats: fileInputName.includes(
                                                "content"
                                            )
                                                ? true
                                                : false
                        ...
                        Promise.all(datCreatePromises)
                            .then(datResponses => {
                                ...
                                let contentJson: AOContent = AOContent.fromObject(
                                    {
                                        ...
                                        contentType: "VOD",
                                        contentLicense:
                                            args.inputs.contentLicense,
                                        contentAttribution:
                                            args.inputs.contentAttribution,
                                        ...
                                        profitSplitPercentage:
                                            args.inputs.profitSplitPercentage,
                                ...
                                Promise.all(storagePromises)
                                    .then(
                                        (results: Array<IAORouterMessage>) => {
                                            ...
                                            const folderMovePromises: Array<
                                                Promise<any>
                                            > = [
                                                context.router.send(
                                                    "/fs/move",
                                            ...
                                            Promise.all(folderMovePromises)
                                                .then(() => {
                                                    resolve(contentJson);
                                                    context.router.send(
                                                        "/db/logs/insert",
