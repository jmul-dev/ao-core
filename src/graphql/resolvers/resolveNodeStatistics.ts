...
export default (obj: AOContent, args: any, context: IGraphqlResolverContext, info: any) => {
    return new Promise((resolve, reject) => {
        const p2pStatsPromise = context.router.send('/p2p/stats', null, {ignoreLogging: true})
        const ethStatsPromise = context.router.send('/eth/stats', null, {ignoreLogging: true})
        Promise.all([p2pStatsPromise, ethStatsPromise]).then((results: Array<IAORouterMessage>) => {
